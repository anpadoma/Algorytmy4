...
/*
    // Zmienia klucz powi¹zany z indeksem k (lub wstawia dane, jeœli indeks k nie wystêpuje w kolejce)
    ...
    public Key get(int k) {
        if (!contains(k)) throw new RuntimeException("Element nie wystêpuje w kolejce");
...
*/
    ...
    public void increase(int k, Key key) {
        if (!contains(k)) throw new RuntimeException("Element nie wystêpuje w kolejce");
        if (keys[k].compareTo(key) >= 0) throw new RuntimeException("Niedozwolone zwiêkszanie");
        keys[k] = key;
        sink(qp[k]);
    }
